...
ll <- read_mip_tables(scen_name, mod_dirs, 
                      tab = "^resource.capacity.csv$")
lapply(ll, colnames)
...
ll <- read_mip_tables(scen_name, mod_dirs, tab = "^dispatch.csv(.gz)?$")
lapply(ll, colnames)
lapply(ll, anyDuplicated)
...
capacity_factors <- dispatch |> 
  # filter(value > 0) |>
  # left_join(filter(resources, unit != "MWh"), 
  #           relationship = "many-to-one") |>
  right_join(filter(resources, unit != "MWh"), 
            relationship = "many-to-one") |>
  mutate(
  ...
  filter(cf_end >= 0.01 | cf_end <= -.01) # drop barrier's crumbs
...
tot_hrs <- max(dispatch$hour, na.rm = T) + 1; tot_hrs
stopifnot(tot_hrs == 168 || tot_hrs == 8760)
...
capacity_factors_mean <- capacity_factors |> 
  ...
  summarise(
    cf_end = round(sum(cf_end, na.rm = T) / tot_hrs, 3), 
    .groups = "drop") |>
  pivot_wider(names_from = model, values_from = cf_end) |>
  as.data.table()
summary(select(capacity_factors_mean, GenX:USENSYS))
...
int_lab <- c(
  "-Inf < cf < 0",
  paste0(int_seq[-length(int_seq)], " < cf <= ", c(int_seq[-1])),
  "1 < cf < Inf"
)

cf_int <- capacity_factors_mean |>
  pivot_longer(cols = GenX:USENSYS, names_to = "model", values_to = "cf_mean") |>
  mutate(interval = cut(cf_mean, c(-Inf, int_seq, Inf), labels = int_lab, 
                   ordered_result = T))
      ...
      tabItem(
        ...
        fluidRow(
          fluidRow(
            ...
            #   width = 2,
            #   pickerInput(
            #     "intervalInput", "CF Intervals:",
            #     choices = unique(capacity_factors$interval),
            #     options = list(`actions-box` = TRUE),
            #     selected = unique(capacity_factors$interval),
            #     multiple = TRUE
            #   )
            # )            
          ),
...
server <- function(input, output) {
  ...
  observe({
    ...
    # print("Interval Input:"); print(input$intervalInput)
  ...
  filtered_cf <- reactive({
    ...
    cf_filtered <- capacity_factors |>
      ...
      # filter(interval %in% input$intervalInput) |>
      filter(resource_name %in% input$resourceInput)
    cf_filtered
  }) 

  # observeEvent(input$zoneInput, {
  #   updated_data <- capacity_factors |>
  #     filter(zone %in% input$zoneInput)
  #   
  #   updatePickerInput(
  #     session, 
  #     inputId = "resource_name",
  #     choices = unique(updated_data$resource_name)
  #   )
  # })
