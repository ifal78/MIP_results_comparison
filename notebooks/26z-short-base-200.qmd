...
<!-- ```{python}
# | title: Regional Existing Capacity
ex_reg_cap_sum = (
    cap.query("existing==True and unit=='MW'")
    .groupby(["agg_zone", "model", "planning_year", "tech_type"], as_index=False)[
        "end_value"
    ]
    .sum()
    .pivot(columns="model", index=["agg_zone", "tech_type", "planning_year"])
    .reset_index()
)
cols = ex_reg_cap_sum.columns
ex_reg_cap_sum.columns = list(cols.get_level_values(0)[:3]) + list(
    cols.get_level_values(1)[3:]
)
display(Markdown(ex_reg_cap_sum.to_markdown(index=False)))
``` -->
