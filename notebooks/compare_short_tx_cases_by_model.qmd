...
chart = chart_total_cap(
    cap.query("planning_year==2050"), x_var="case", row_var="model", col_var="agg_zone"
)
chart
```
...
chart = chart_tx_expansion(
    tx_all, x_var="case", facet_col=None, col_var="line_name", row_var="model"
)
chart
```
...
chart = chart_tx_expansion(tx_all, x_var="case", facet_col=None, col_var="model")
chart
```
...
chart = chart_op_cost(op_costs, "case", "model", "planning_year")
chart
```
```{python}
# | title: Operational NSE
chart = chart_op_nse(op_nse, x_var="case", col_var="model", row_var="planning_year")
chart
```

```{python}
# | title: Generation
chart = chart_total_gen(op_gen, op_cap, "case", "model", "planning_year")
chart
```
...
chart = chart_op_emiss(
    op_emiss, x_var="case", col_var="model", row_var="planning_year", color="tech_type"
)
chart
```
...
chart = chart_op_emiss(op_emiss, x_var="case", col_var="model", row_var="planning_year", color="zone")
chart
```